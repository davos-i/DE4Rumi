...
#'
#' Internal function to use within [auto_generate_DE_results].
#'
...
#'
#' @param top_level_group_individual inherits from [auto_generate_DE_results]
#' @param dds_object DESeq2 data object, inherits from [auto_generate_DE_results]
#' @param gene_annotations Output from [annotate_gene_ensembl]
...
#'   are exported to \code{expor_dir}
#' @param export_dir string. "./" indicates relative to working directory. IF
#'   this specified directory doesn't exist, it will be created.
#' @param results_contrast_factor inherits from [auto_generate_DE_results]
#' @param top_level_colname inherits from [auto_generate_DE_results].
#'   non-string. Name of \code{colData} column with top level factor, which is
#'   normally Tissue Region.
#' @param whole_data_normalisation inherits from [auto_generate_DE_results]
#'
#'
...
if(whole_data_normalisation == FALSE){
  results_contrast_factor_string <-
    rlang::enquo(results_contrast_factor) %>%
    rlang::as_label()
  ...
  #this is the 'factor' that it normalises by below
  dds_temp <- BiocGenerics::estimateSizeFactors(dds_object)
  ...
  ############################################################################ #
  #export tables of VST and log2 normalised data
  if(export_tables == TRUE){
    ...
    data.table::fwrite(log2norm_a,
                       file = paste0(export_dir,
                                     top_level_group_individual,
                                     "_log2_of_DESEQ_internal_normalisedcounts_",
                                     format(Sys.time(), "%Y%m%d_%H%M"),
                                     ".txt"),
                       row.names = F)
  ...
  pcaData <- DESeq2::plotPCA(vsd,
                             intgroup=results_contrast_factor_string,
                             returnData=TRUE,
                             ntop = 500)

  percentVar <- round(100 * attr(pcaData, "percentVar"))

  PCA_plot <- ggplot2::ggplot(pcaData,
                              ggplot2::aes(.data$PC1, .data$PC2, fill=!!rcf)) +
  ...
  topVarGenes <-
    BiocGenerics::order(genefilter::rowVars( SummarizedExperiment::assay(vsd) ), decreasing=TRUE )[1:30]
