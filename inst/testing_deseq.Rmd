...
test_data2 <- test_data %>% SummarizedExperiment::subset(
  select = SummarizedExperiment::colData(test_data)[["Region"]] %in% c("ARC", "LIV"))

full_norm <- 
  auto_generate_DE_results(se_data = test_data2,
                           top_level_colname = Region,
                           sample_colname = sample_names,
                           samples_to_remove = NA,
                           DESeq2_formula_design = ~Region_Diet,
