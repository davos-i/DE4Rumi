...
#DE_dataframes_list %>% names()


f_test <- function(DE_data){
  print(names(DE_data))
  f2 <- function(x){
    print("test")
    print(names(x))
  }
  purrr::map(DE_data, f2)
   top_names <- names(DE_data) %>%
    stringr::str_extract(pattern = "^[^_]*")
   print(top_names)
}

test_out <- f_test(DE_dataframes_list)



RIF_output <- 
  calculate_RIF(DE_data_list = DE_dataframes_list,
                TFs = NA,
                norm_exp_data = full_norm$Whole_data_normalisation_output$normalised_data$vsd,
