...
For multiple iterations it can take a long time to run.
```{r include=FALSE}
...
DE_out <- DE_out %>% unlist(recursive = FALSE)
```
...
saveRDS(DE_out, file = "./outputs/DE_out_file.rds")
...
DE_dataframes <- 
  DE_out %>% 
  purrr::map("DESeq2_annot_df") %>% 
  unlist(recursive = FALSE)

#access individual tables by using $ 
#e.g.
DE_dataframes$`ARC_DESeq2_Output.ARC_HCP.HP.UMEI vs ARC_LCP.LP.UMEI` %>% head()

#Can also search with a string e.g. "ARC"
ARC_DE  <- DE_dataframes %>% 
  {.[grepl("ARC", names(.))]} 

ARC_DE$`ARC_DESeq2_Output.ARC_HCP.HP.RMEI vs ARC_HCP.HP.UMEI` %>% head()
...
PIF_dataframes <- 
 ...
 #access individual tables by using $ 
#e.g.
PIF_dataframes$`ARC_DESeq2_Output.PIF - ARC_LCP.HP.UMEI vs ARC_HCP.LP.UMEI`
```
...
#access individual tables by using $ 
#e.g.
DE_results_objects$`ARC_DESeq2_Output.ARC_HCP.HP.RMEI vs ARC_HCP.HP.UMEI` 
